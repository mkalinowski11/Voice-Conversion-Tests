...
if __name__ == "__main__":
    ...
    model_phase = TransformerModel(embed_dim = config["model"]["embed_dim"],
        num_heads = config["model"]["num_heads"],
        n_enc_blcks = config["model"]["n_enc_blcks"],
        n_dec_blcks = config["model"]["n_dec_blcks"],
        device = "cpu")
    ...
    with torch.no_grad():
        # STFT processing
        source_target_amp = model_stft(normalized_source_amp[:min_dim, :], normalized_target_amp[:min_dim, :])
        target_source_amp = model_stft(normalized_target_amp[:min_dim, :], normalized_source_amp[:min_dim, :])
        # Phase processing
        source_target_phase = model_phase(normalized_source_phase[:min_dim, :], normalized_target_phase[:min_dim, :])
        target_source_phase = model_phase(normalized_target_phase[:min_dim, :], normalized_source_phase[:min_dim, :])
