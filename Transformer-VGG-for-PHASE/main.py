...
def main(config):
    ...
    optimizer_phase = torch.optim.Adam(
        phase_model.parameters(), lr=config["optimizer"]['lr'],
        betas=(config["optimizer"]['beta1'], config["optimizer"]['beta2']),
        weight_decay=config["optimizer"]['weight_decay']
    )
    #
    ...
    for iteration in pbar:
        ...
        meta_phase = train_step(phase_model, optimizer_phase, phase, l1_loss, mse_loss)
        #
        ...
        if (iteration + 1) % config["save_frequency"] == 0 or (iteration + 1) == n_iterations:
            ...
            torch.save(phase_model.state_dict(), f'model_save_iter_{iteration + 1}_PHASE.ckpt')
            print("Saving model ===>>>")
